...
use crate::core::lfs::LFS_THRESHOLD;
use crate::core::defaults::common::default_polling_interval;
...
impl TwinkleRepository {
    pub fn polling_interval(&self) -> Duration {
        let default = default_polling_interval().as_secs();

        match self.git.config_get(&key(K_POLLING_INTERVAL)) {
            Some(output) => Duration::from_mins(parse_polling_interval(&output)),
...
impl TwinkleRepository {
    ...
    pub fn lfs_size_threshold(&self) -> u64 {
        match self.git.config_get(&key(K_LFS_SIZE_THRESHOLD)) {
            Some(output) => parse_lfs_size(&output),
            None => LFS_THRESHOLD,
        }
    }
