...
) -> Result<(), Box<dyn Error>>
{
    ...
    loop {
        ...
        let lfs_enabled = repo.lfs_enabled(); // TODO: Move to core::lfs?
        ...
        if let Some(message) = pretty::format_commit_message(&status) {
        ...
        let status = repo.git.status(GitStatusFilter::All)?;
        if !has_unpushed_commits(repo) && status.is_empty() {
            break;
        }
